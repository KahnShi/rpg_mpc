...
class MpcHydrusController {
 public:
  ...
  static_assert(kStateSize == 13,
    "MpcHydrusController: Wrong model size. Number of states does not match.");
  static_assert(kInputSize == 4,
    "MpcHydrusController: Wrong model size. Number of inputs does not match.");

  MpcHydrusController(const ros::NodeHandle & nh, const ros::NodeHandle & pnh);
  MpcHydrusController() : MpcHydrusController(ros::NodeHandle(), ros::NodeHandle("~")) {}
